...
aig_optimization_experiments/
  ...
  sat_refinement_placeholder.py   filters top candidates for future SAT verification
...
## SAT refinement pipeline (current status)

The current pipeline is:

```
simulation ranking → candidate list → (placeholder) SAT check flag
```

`sat_refinement_placeholder.py` filters candidates with `combined_score >= 0.85` and
writes `results/sat_refinement_candidates.csv`. The actual SAT verification is **not yet
implemented** — this placeholder marks where the SAT step will go in the next iteration.

The intended future pipeline is:

```
simulation ranking → SAT equivalence check → counterexample-guided ranking refinement
```
...
- SAT refinement is not yet implemented.
...
3. Implement SAT-based candidate verification after the top-K ranking step.
...
> motivates a region-based matching approach as the next step.
