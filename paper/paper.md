...
future-work rows move up in evidence level today: exact-node source-blind
placement still promotes 0/56, but bounded source-blind window/expression
placement promotes 14/56 graph-active CEC-backed recoveries from the 20 rows
with prior semantic counterpart evidence; 22/48 necessity-first rows become
...
| Materialized frontier replay pairs | 36 / 36 | Fresh utility rows are repaired into checkpoint-derived source/optimized BLIF pairs with source-function targets, output equivalence, and CEC-backed replay provenance. |
| Proof-carrying virtual anchor synthesis | 56 / 56 | Latent source cuts and PI truth-table anchors synthesize source-derived virtual anchors with JSON certificates; constructive expanded-support rewrites recover the 6 former identical-driver rows after replay-pair repair removes the missing-artifact blocker. |
...
targets are lifting PI truth-table anchors into reusable source-cut
decompositions, compressing replay-pair consumers toward minimal residual
interfaces, broader CEGIS grammars with completeness proofs for selected
