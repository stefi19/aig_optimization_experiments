...
claims after logic optimization without mixing evidence levels or denominators.
...
diagnostics. External RTL claims remain out of scope; the repository does not
claim a redistributable RTL corpus or pinned Yosys lowering flow.
...
across evidence levels honestly. The most important directions are:

- infer source-side counterparts without controlled access;
- synthesize graph-active rewrites from the 31/48 compact exact generated
  interfaces;
- extend blind CEGIS templates and prove when bounded grammars are complete for
  selected operator families;
- add a pinned redistributable RTL corpus with a Yosys lowering flow and
  source-location metadata;
- integrate ODC-aware placement with explicit graph-activity and global CEC
  obligations;
- add machine-checkable proof objects for exact locality certificates beyond
  replayable CSV evidence.
