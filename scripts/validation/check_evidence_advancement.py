#!/usr/bin/env python3
"""Validate the evidence advancement layer as a reviewer would.

The checks in this file intentionally duplicate the important semantic claims
instead of trusting the builder's status strings.  Every proof-carrying virtual
anchor must replay from source-side artifacts, match the optimized target truth
vector, satisfy the recorded support bound, emit a graph-active rewrite, and
pass the two CEC scopes that connect source, rewrite, and optimized target.  The
validator also treats denominator drift as a failure: a cleaner-looking result
with fewer rows is not acceptable unless the paper claim changes with it.
"""
...
def _check_materialized_replay_pairs(rows: list[dict[str, str]], errors: list[str]) -> None:
    """Check the 36 synthesized replay pairs used by virtual anchors.

    These rows are the bridge from compact fresh utility labels to concrete BLIF
    artifacts.  The validator re-derives their linkage to active-source rows,
    semantic frontier transitions, checkpoint hashes, boundary supports, target
    truth vectors, and primary-output equivalence.  That wide check is what makes
    the new 56/56 result proof-carrying rather than merely table-complete.
    """
    if len(rows) != 36:
...
def _check_latent_source_cut_bank(rows: list[dict[str, str]], errors: list[str]) -> dict[str, dict[str, str]]:
    """Replay each latent source cut from its source artifact.

    The builder is allowed to synthesize virtual anchors, but it is not allowed
    to synthesize evidence out of thin air.  This routine evaluates every stored
    expression against the committed source BLIF, recomputes hashes and support,
    and confirms the canonical metadata needed for later decomposition checks.
    """
...
def _check_virtual_decomposition_source(
    ...
    """Verify that one optimized target is explained by source-side evidence."""
    target_instance_id = _stable_id(index, source_row.get("target_id", ""))
...
def _check_virtual_anchor_certificate(
    ...
    """Validate the JSON proof object behind one certificate row."""
