...
def validate_blif(path: Path) -> list:
    """Return a list of validation error strings (empty list means the file is OK)."""
    errors = []
    try:
...
def import_iscas85(input_dir: Path, output_dir: Path) -> None:
    ...
    Does NOT download anything.  Only copies files that already exist on disk
    and pass basic BLIF validation.
    """
    ...
    for bf in blif_files:
        ...
        shutil.copy2(bf, output_dir / bf.name)
        s = _parse_blif_stats(bf)
        print(
            f"  {bf.name:<25}  {s['inputs']:>7}  {s['outputs']:>8}"
            f"  {s['nodes']:>6}  ✓ copied"
        )
        ok_count += 1
