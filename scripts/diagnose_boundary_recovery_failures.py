"""Compatibility adapter for :mod:`scripts.boundary.diagnose_boundary_recovery_failures`.
...
long-standing `python scripts/diagnose_boundary_recovery_failures.py` command used in old notes, Makefile
targets, CI logs, and external reproductions while delegating all real work to
`python -m scripts.boundary.diagnose_boundary_recovery_failures`.
"""
...
_TARGET_MODULE = "scripts.boundary.diagnose_boundary_recovery_failures"
